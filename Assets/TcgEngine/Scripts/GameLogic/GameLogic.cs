    ...
    public class GameLogic
    {
        ...
        public virtual void StartTurn()
        {
            ...
            foreach (Player player in game_data.players)
            {
                ...
                //Cards draw
                if (game_data.turn_count > 1 || player.player_id != game_data.first_offense_player)
                {
        ...
        public virtual void StartNextTurn()
        {
            ...
            game_data.current_offsense_player = (game_data.current_offsense_player + 1) % game_data.settings.nb_players;

            if (game_data.current_offsense_player == game_data.first_offense_player)
        ...
        public virtual void RevealNewPlayers()
        {
            ...
            //TODO: how do we show the players?
...
/*        public virtual void StartMainPhase()
        ...
        public virtual void EndGame(int winner)
        {
            if (game_data.state != GameState.GameEnded)
            {
                ...
                game_data.selector = SelectorType.None;
                game_data.current_offsense_player = winner; //Winner player
                resolve_queue.Clear();
...
/*            if (puzzle != null)
        ...
        public virtual void DamageCard(Card attacker, Card target, int value, bool spell_damage = false)
        {
            ...
            if (!spell_damage && extra > 0 && attacker.player_id == game_data.current_offsense_player && attacker.HasStatus(StatusType.Trample))
                tplayer.hp -= extra;
        ...
        public virtual bool TriggerSecrets(AbilityTrigger secret_trigger, Card trigger_card)
        {
            ...
            for (int p = 0; p < game_data.players.Length; p++)
            {
                if (p != game_data.current_offsense_player)
                {
